...
export const getHttpTreeList = (params: { projectId: string }) => {
  return http.post<Http.ResTree>(PORT1 + `/http/list`, params);
};
...
export const updateHttpConfig = (params: Http.ReqUpdate) => {
  return http.post<Http.ResConfig>(PORT1 + `/http/update`, params);
};

//添加接口配置
export const addHttpConfig = (params: Http.ReqAdd) => {
  return http.post<Http.ResConfig>(PORT1 + `/http/add`, params);
};

//删除接口配置
export const deleteHttpConfig = (params: { apiId: number }) => {
  return http.post<Http.ResConfig>(PORT1 + `/http/delete`, params);
};
